...
'-----------------------------
'04/05/2022 Revision 1.0.2 AxioUK
            'Improved MousePointerHand Property
'22/05/2022 Revision 1.0.3 AxioUK
            'adjust to fit AxFramework
'---------------------------------------------------------------------------------------
...
Private m_ParteZeros        As Long    'Yaco

Dim m_MousePointerHands As VisiblePointer
Dim hCur As Long
...
Property Get ParteZeros() As Long
'Agregado por YAco
 ParteZeros = m_ParteZeros
End Property
 
 Property Let ParteZeros(ByVal Value As Long)
'Agregado por YAco
    m_ParteZeros = Value
    PropertyChanged "ParteZeros"
End Property
...
Private Sub UserControl_InitProperties()
    ...
    m_BackColor = &H84541A   ' vbWindowBackground
    m_BackColorOnFocus = &H84541A    'vbWindowBackground
    m_BorderColor = &HB87F3F    'vbActiveBorder
    m_BorderColorOnFocus = &HFFFFFF    'vbHighlight
    m_ParentBackColor = Ambient.BackColor
    UserControl.ForeColor = &HFFFFFF    'Ambient.ForeColor
...
Private Sub UserControl_GotFocus()
    ...
    If m_InputType = IT_Numeric Then
      Dim mSel As Long
      If m_ParteZeros > 0 And Val(Me.Text) > 0 Then
          mSel = Me.SelStart
          Me.Text = Val(Me.Text)
          Me.SelStart = mSel
      End If
...
Private Sub UserControl_LostFocus()
    ...
    '/------------------Yaco
    If m_InputType = IT_Decimal Then
       Me.Text = FormatNumber(Me.Text, m_ParteDecimal, vbTrue)
    End If
    
    If m_InputType = IT_Numeric Then
      If m_ParteZeros > 0 And Val(Me.Text) > 0 Then
          Me.Text = Format(Me.Text, String(m_ParteZeros, "0"))
      End If
    End If
    '----------------------/
    '/-AxioUK----------Formato-RUT-------
...
Private Sub UserControl_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    If X < m_Rect.Left And X > 0 And Y > 0 And Y < m_Rect.Bottom Then
        If m_MousePointerHands = vpOnLeftIcon Or m_MousePointerHands = vpOnBothIcon Then
    ...
    ElseIf (X > m_Rect.Left + m_Rect.Right) And X < UserControl.ScaleWidth And Y > 0 And Y < m_Rect.Bottom And (m_RightButtonStyle <> RS_SpinButton) Then
        If m_MousePointerHands = vpOnRightIcon Or m_MousePointerHands = vpOnBothIcon Then
            If Ambient.UserMode Then
                UserControl.MousePointer = vbCustom
                UserControl.MouseIcon = GetSystemHandCursor
            End If
...
Public Property Let MousePointerHands(ByVal NewValue As VisiblePointer)
    m_MousePointerHands = NewValue
    PropertyChanged "MousePointerHands"
End Property

Public Property Get MousePointerHands() As VisiblePointer
...
Private Sub UserControl_ReadProperties(PropBag As PropertyBag)
    With PropBag
        ...
        m_ParteZeros = .ReadProperty("ParteZeros", 0)      'yaco
        m_MousePointerHands = .ReadProperty("MousePointerHands", 0) 'yaco
        UserControl.MousePointer = .ReadProperty("MousePointer", vbArrow) 'yaco
        UserControl.MouseIcon = .ReadProperty("MouseIcon", Nothing) 'yaco
...
Private Sub UserControl_WriteProperties(PropBag As PropertyBag)
    ...
    With PropBag
        ...
        .WriteProperty "ParteZeros", m_ParteZeros    'yaco
        .WriteProperty "MousePointerHands", m_MousePointerHands, 0  'yaco
        .WriteProperty "MouseIcon", UserControl.MouseIcon, Nothing      'yaco
        .WriteProperty "MousePointer", UserControl.MousePointer, vbArrow 'yaco
...
Private Sub CalculateCaptionSize()
    ...
    If m_Caption <> vbNullString Or m_Caption = "" Then
                    ...
                    If lParam = hEdit Then
                        ...
                        If TextBoxChangeFrozen = False Then
                            ...
                            ElseIf m_InputType = IT_LettersOnly Then
                                ...
                                TextBoxChangeFrozen = False
                                Me.SelStart = Sel
                            ElseIf m_InputType = IT_Decimal Then
                                m_Text = Me.Text
                                Sel = Me.SelStart
                                For i = 1 To Me.TextLenght
                                    iChar = Asc(Mid(m_Text, i, 1))
